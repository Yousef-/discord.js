...
callback = email;email = null;password = null;}return this.internal.loginWithToken(token,email,password).then(dataCallback(callback),errorCallback(callback));}; /**
	 ...
	 *     .then(token => console.log(token))
	 *     .catch(err => console.log(err));
	 */Client.prototype.loginAsBot = function loginAsBot(token){var callback=arguments.length <= 1 || arguments[1] === undefined?function() /*err, token*/{}:arguments[1];return this.internal.loginWithToken('Bot ' + token,null,null).then(dataCallback(callback),errorCallback(callback));}; /**
